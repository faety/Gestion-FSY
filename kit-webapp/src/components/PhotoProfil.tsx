...
import { demanderSignaturePhotoProfil, enregistrerMaPhoto, supprimerMaPhoto } from "@/lib/actions";

// Envoi d'une image directement du navigateur vers Cloudinary.
//
// 1. Réduction dans le navigateur (canvas) : on n'envoie pas 4 Mo depuis un
//    téléphone quand 200 Ko suffisent.
// 2. Le serveur signe les paramètres (dossier, horodatage, type) — la clé
//    secrète ne quitte jamais le serveur.
// 3. Le navigateur envoie chez Cloudinary, récupère le public_id, et l'action
//    serveur l'enregistre après avoir vérifié qu'il est dans le bon dossier.
//
// Ce composant gère un portrait carré ; pour d'autres usages (photos de
// rapport, pièces jointes), copier `reduire` et `envoyerChezCloudinary`.
...
export async function envoyerChezCloudinary(
  fichier: string | Blob,
  sig: NonNullable<Awaited<ReturnType<typeof demanderSignaturePhotoProfil>>>
): Promise<string> {
...
export function PhotoProfil({
  ...
  async function choisir(fichier: File) {
    ...
    try {
      ...
      const publicId = await envoyerChezCloudinary(dataUrl, sig);
      const r = await enregistrerMaPhoto(publicId);
    ...
    <div className="flex items-center gap-4">
      <div className="shrink-0">
        {apercu ? (
          ...
          <img src={apercu} alt="" className="w-20 h-20 rounded-full object-cover bg-slate-100" />
        ) : (
          <span className="w-20 h-20 rounded-full bg-marque-claire text-marque-sombre inline-flex items-center justify-center text-2xl font-bold">
            {init || "?"}
          </span>
        )}
      </div>
          ...
          <>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="bg-marque hover:bg-marque-sombre text-white rounded-lg px-4 py-2 text-sm font-semibold disabled:opacity-50"
              >
            ...
            <p className="text-xs text-slate-500">L'image est recadrée en carré et réduite avant l'envoi.</p>
          </>
        ) : (
          <p className="text-sm text-slate-500">L'envoi de photos n'est pas configuré sur ce déploiement.</p>
        )}

        {erreur && (
          <p className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-2">{erreur}</p>
        )}
      </div>
    </div>
  );
