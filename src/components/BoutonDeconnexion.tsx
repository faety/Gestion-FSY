...
// Déconnexion précédée d'une confirmation.
//
// Le bouton est en haut à droite, juste à côté du portrait et de la navigation :
// on l'atteint du pouce sans le vouloir. Se retrouver déconnecté au milieu d'un
// pointage, puis devoir retaper un mot de passe debout dans un car, coûte assez
// cher pour justifier une question.
export function BoutonDeconnexion({ prenom }: { prenom: string }) {
  const [ouvert, setOuvert] = useState(false);
  const annuler = useRef<HTMLButtonElement>(null);

  // Échap ferme, et le focus part sur « Rester connecté » : la confirmation ne
  // doit pas être le choix par défaut d'une tape sur Entrée.
  useEffect(() => {
    if (!ouvert) return;
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-slate-600 mt-1">
              Il faudra saisir de nouveau votre adresse et votre mot de passe pour revenir.
            </p>
